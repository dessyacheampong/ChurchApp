...
const ChurchManagementSystem = () => {
  ...
  const getAccumulatedDuesOwed = (member: any) => {
    ...
    return { totalExpected, totalPaid, owed: Math.max(0, totalExpected - totalPaid) };
  };

  // Per-year dues summary for the member detail sheet
  const getMemberDuesSummary = (member: any) => {
    ...
    const years = [];
    for (let y = joinYear; y <= todayYear; y++) {
      const startM = y === joinYear ? joinDate.getMonth() : 0;
      const endM = y === todayYear ? today.getMonth() : 11;
      const memberDues = getDuesForMemberYear(member.id, y);
      const months = [];
      let yearPaid = 0;
      let yearExpected = 0;
      for (let m = startM; m <= endM; m++) {
        const paid = parseFloat(memberDues.months[monthKeys[m]]) || 0;
        yearPaid += paid;
        yearExpected += MONTHLY_DUES;
        months.push({ label: monthLabels[m], paid, expected: MONTHLY_DUES });
      }
      years.push({ year: y, months, yearPaid, yearExpected, yearOwed: Math.max(0, yearExpected - yearPaid) });
    }
    return years;
  };
  ...
  // All Sundays from Jan 4 2026 through May 17 2026
  const SUNDAYS_2026 = (() => {
    const sundays = [];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
          ...
          <div>
            ...
            <div className="sm:hidden space-y-3">
              {filteredMembers.map((member) => (
                <div key={member.id} className="bg-white rounded-xl shadow-md p-4 border border-gray-100 cursor-pointer active:bg-blue-50" onClick={() => setSelectedMember(member)}>
            ...
            <div className="hidden sm:block bg-white rounded-2xl shadow-xl overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredMembers.map((member) => (
                    <tr key={member.id} className="hover:bg-blue-50 transition-colors duration-200 cursor-pointer" onClick={() => setSelectedMember(member)}>
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {getMembersForYear(selectedDuesYear).map((member) => {
                            ...
                            const { owed: owedAmount } = getAccumulatedDuesOwed(member);
        ...
        {/* SUMMARY TAB */}
        {activeTab === 'summary' && (() => {
          const years = Array.from(new Set([...weeklySheets.map(w => w.date?.slice(0,4)), String(new Date().getFullYear())])).sort().reverse();
          const filtered = weeklySheets.filter(w => w.date?.startsWith(String(summaryYear)));
          const totals = filtered.reduce((acc: any, w: any) => {
            acc.tithes += parseFloat(w.tithes || 0);
            acc.dues += parseFloat(w.dues || 0);
            acc.funeralContributions += parseFloat(w.funeralContributions || 0);
            acc.sundaySchool += parseFloat(w.sundaySchool || 0);
            acc.bookSales += parseFloat(w.bookSales || 0);
            return acc;
          }, { tithes: 0, dues: 0, funeralContributions: 0, sundaySchool: 0, bookSales: 0 });
          const grossRevenue = totals.tithes + totals.dues + totals.funeralContributions + totals.sundaySchool + totals.bookSales;
          const totalExpenditure = expenditures.filter(e => e.date?.startsWith(String(summaryYear))).reduce((s: number, e: any) => s + parseFloat(e.amount || 0), 0);
          const balance = grossRevenue - totalExpenditure;
          const cats = [
            { key: 'tithes', label: 'Tithes', color: 'blue' },
            { key: 'dues', label: 'Dues', color: 'indigo' },
            { key: 'funeralContributions', label: 'Funeral Contributions', color: 'purple' },
            { key: 'sundaySchool', label: 'Sunday School Offering', color: 'green' },
            { key: 'bookSales', label: 'Book Sales', color: 'orange' },
          ];
          return (
            <div>
              <div className="flex flex-wrap justify-between items-center mb-5 gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Weekly Collections Summary</h2>
                  <p className="text-gray-500 text-sm mt-0.5">Sunday-by-Sunday breakdown of all collections</p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => openModal('weeklySheet')} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-semibold shadow-md">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
                {cats.map(c => (
                  <div key={c.key} className={`bg-${c.color}-50 border border-${c.color}-200 rounded-xl p-3`}>
                    <div className={`text-xs font-semibold text-${c.color}-600 mb-1`}>{c.label}</div>
                    <div className={`text-lg font-bold text-${c.color}-800`}>GH₵{(totals as any)[c.key].toFixed(2)}</div>
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl p-4 text-white">
                  <div className="text-xs font-semibold text-emerald-100 mb-1">Gross Revenue</div>
                  <div className="text-xl font-bold">GH₵{grossRevenue.toFixed(2)}</div>
                </div>
                <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-4 text-white">
                  <div className="text-xs font-semibold text-red-100 mb-1">Total Expenditure</div>
                  <div className="text-xl font-bold">GH₵{totalExpenditure.toFixed(2)}</div>
                </div>
                <div className={`bg-gradient-to-br rounded-xl p-4 text-white ${balance >= 0 ? 'from-blue-600 to-indigo-600' : 'from-orange-500 to-red-600'}`}>
                  <div className="text-xs font-semibold text-blue-100 mb-1">Balance</div>
                  <div className="text-xl font-bold">GH₵{balance.toFixed(2)}</div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase sticky left-0 bg-gray-50 min-w-28">Date</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {SUNDAYS_2026.map(sunday => {
                        const sheet = filtered.find(w => w.date === sunday);
                        const rowTotal = sheet ? (
                          parseFloat(sheet.tithes||0) + parseFloat(sheet.dues||0) +
                          parseFloat(sheet.funeralContributions||0) + parseFloat(sheet.sundaySchool||0) + parseFloat(sheet.bookSales||0)
                        ) : null;
                        return (
                          <tr key={sunday} className={`${sheet ? 'hover:bg-blue-50' : 'opacity-40'} transition-colors`}>
                            <td className="px-4 py-3 sticky left-0 bg-white font-medium text-gray-800">
                              {new Date(sunday + 'T00:00:00').toLocaleDateString('en-GH', { day:'2-digit', month:'short' })}
                            </td>
                            {cats.map(c => (
                              <td key={c.key} className="px-4 py-3 text-right text-gray-700">
                                {sheet ? `GH₵${parseFloat((sheet as any)[c.key]||0).toFixed(2)}` : '—'}
                              </td>
                            ))}
                            <td className="px-4 py-3 text-right font-bold text-gray-900">
                              {rowTotal !== null ? `GH₵${rowTotal.toFixed(2)}` : '—'}
                            </td>
                            <td className="px-4 py-3 text-center">
                              {sheet ? (
                                <div className="flex justify-center gap-2">
                                  <button onClick={() => openModal('weeklySheet', sheet)} className="text-blue-600 hover:text-blue-900"><Edit className="w-4 h-4" /></button>
                                  <button onClick={() => handleDelete('weeklySheet', sheet.id)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button>
                                </div>
                              ) : (
                                <button onClick={() => { setWeeklySheetForm({ date: sunday, tithes: '', dues: '', funeralContributions: '', sundaySchool: '', bookSales: '' }); setModalType('weeklySheet'); setEditingItem(null); setShowModal(true); }} className="text-xs text-blue-500 hover:text-blue-700 border border-blue-300 px-2 py-0.5 rounded">+ Record</button>
                              )}
                            </td>
                          </tr>
                        );
                      ...
                      <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                        ...
                        {cats.map(c => (
                          <td key={c.key} className="px-4 py-3 text-right text-gray-900">GH₵{(totals as any)[c.key].toFixed(2)}</td>
                        ))}
                        <td className="px-4 py-3 text-right text-emerald-700">GH₵{grossRevenue.toFixed(2)}</td>
                        <td></td>
                      </tr>
                    </tbody>
                  </table>
        ...
        {activeTab === 'expenditure' && (() => {
          const sortedExp = [...expenditures].sort((a, b) => a.date?.localeCompare(b.date));
          const totalRevenue = weeklySheets.reduce((s: number, w: any) => {
            return s + parseFloat(w.tithes||0) + parseFloat(w.dues||0) + parseFloat(w.funeralContributions||0) + parseFloat(w.sundaySchool||0) + parseFloat(w.bookSales||0);
          }, 0);
          let runningBalance = totalRevenue;
          const rows = sortedExp.map((e: any) => {
          ...
          const finalBalance = totalRevenue - totalExp;
          return (
            <div>
              <div className="flex flex-wrap justify-between items-center mb-5 gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Expenditure</h2>
                  <p className="text-gray-500 text-sm mt-0.5">All expenses with running balance</p>
                </div>
                <button onClick={() => openModal('expenditure')} className="bg-gradient-to-r from-red-600 to-rose-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 text-sm font-semibold shadow-md">
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                  <div className="text-xs font-semibold text-emerald-600 mb-1">Total Revenue</div>
                  <div className="text-xl font-bold text-emerald-800">GH₵{totalRevenue.toFixed(2)}</div>
                </div>
                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                  <div className="text-xs font-semibold text-red-600 mb-1">Total Spent</div>
                  <div className="text-xl font-bold text-red-800">GH₵{totalExp.toFixed(2)}</div>
                </div>
                <div className={`rounded-xl p-4 border ${finalBalance >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-orange-50 border-orange-200'}`}>
                  <div className={`text-xs font-semibold mb-1 ${finalBalance >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>Balance</div>
                  <div className={`text-xl font-bold ${finalBalance >= 0 ? 'text-blue-800' : 'text-orange-800'}`}>GH₵{finalBalance.toFixed(2)}</div>
                </div>
              </div>

              {/* Opening balance row then expenditure rows */}
              <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-right text-xs font-bold text-gray-500 uppercase">Balance After</th>
                        <th className="px-4 py-3 text-center text-xs font-bold text-gray-500 uppercase">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      ...
                      <tr className="bg-emerald-50">
                        ...
                        <td className="px-4 py-3 text-right font-bold text-emerald-700">GH₵{totalRevenue.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right font-bold text-emerald-700">GH₵{totalRevenue.toFixed(2)}</td>
                        <td></td>
                      </tr>
                      {rows.length === 0 && (
                        <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-400 italic">No expenditures recorded yet</td></tr>
                      )}
                      {rows.map((e: any) => (
                        <tr key={e.id} className="hover:bg-red-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-red-700">- GH₵{parseFloat(e.amount).toFixed(2)}</td>
                          <td className="px-4 py-3 text-right font-bold">
                            <span className={e.balanceAfter >= 0 ? 'text-blue-700' : 'text-orange-700'}>GH₵{e.balanceAfter.toFixed(2)}</span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <div className="flex justify-center gap-2">
                              <button onClick={() => openModal('expenditure', e)} className="text-blue-600 hover:text-blue-900"><Edit className="w-4 h-4" /></button>
                              <button onClick={() => handleDelete('expenditure', e.id)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button>
                            </div>
                          </td>
                        </tr>
                      ))}
                      ...
                      {rows.length > 0 && (
                        <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
                          ...
                          <td className="px-4 py-3 text-right text-red-700">- GH₵{totalExp.toFixed(2)}</td>
                          <td className="px-4 py-3 text-right">
                            <span className={`text-lg ${finalBalance >= 0 ? 'text-blue-800' : 'text-orange-700'}`}>GH₵{finalBalance.toFixed(2)}</span>
                          </td>
                          <td></td>
                        </tr>
                      )}
          ...
          <div className="bg-white w-full sm:max-w-2xl sm:rounded-2xl rounded-t-2xl shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-5 py-4 rounded-t-2xl flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-white font-bold text-base">{selectedMember.name}</div>
                  <div className="text-blue-100 text-xs">{selectedMember.ministry || 'No Ministry'} · {selectedMember.status}</div>
                </div>
              </div>
              <button onClick={() => setSelectedMember(null)} className="text-white text-xl font-bold w-8 h-8 flex items-center justify-center rounded-full hover:bg-white hover:bg-opacity-20">✕</button>
            </div>
            ...
            <div className="p-4 space-y-4">
              ...
              {/* Dues Summary */}
              {(() => {
                const { totalExpected, totalPaid, owed } = getAccumulatedDuesOwed(selectedMember);
                const duesSummary = getMemberDuesSummary(selectedMember);
                return (
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      <div className="bg-blue-50 rounded-lg p-3 text-center">
                        ...
                        <div className="font-bold text-blue-800 text-sm">GH₵{totalExpected.toFixed(2)}</div>
                      </div>
                      <div className="bg-green-50 rounded-lg p-3 text-center">
                        <div className="text-xs text-green-600 mb-1">Paid</div>
                        <div className="font-bold text-green-800 text-sm">GH₵{totalPaid.toFixed(2)}</div>
                      </div>
                      <div className={`rounded-lg p-3 text-center ${owed === 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                        <div className={`text-xs mb-1 ${owed === 0 ? 'text-green-600' : 'text-red-600'}`}>Owed</div>
                        <div className={`font-bold text-sm ${owed === 0 ? 'text-green-800' : 'text-red-800'}`}>{owed === 0 ? '✓ Clear' : `GH₵${owed.toFixed(2)}`}</div>
                      </div>
                    </div>
                    {/* Per-year breakdown */}
                    {duesSummary.map(ys => (
                      <div key={ys.year} className="mb-3 border border-gray-200 rounded-xl overflow-hidden">
                        <div className="bg-gray-50 px-4 py-2 flex justify-between items-center">
                          <span className="font-semibold text-sm text-gray-700">{ys.year}</span>
                          <div className="flex gap-2 text-xs">
                            <span className="text-green-700 bg-green-100 px-2 py-0.5 rounded-full">Paid GH₵{ys.yearPaid.toFixed(2)}</span>
                            {ys.yearOwed > 0 && <span className="text-red-700 bg-red-100 px-2 py-0.5 rounded-full">Owes GH₵{ys.yearOwed.toFixed(2)}</span>}
                          </div>
                        </div>
                        <div className="grid grid-cols-4 sm:grid-cols-6 gap-1 p-2">
                          {ys.months.map(m => (
                            <div key={m.label} className={`rounded-lg p-1.5 text-center text-xs ${m.paid >= m.expected ? 'bg-green-100 text-green-800' : m.paid > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-50 text-red-700'}`}>
                              <div className="font-medium">{m.label}</div>
                              <div>{m.paid > 0 ? `GH₵${m.paid}` : '—'}</div>
                            </div>
                          ))}
                        </div>
                      </div>
              ...
              {/* Tithes Summary */}
              {(() => {
                const memberTithes = tithes.filter(t => t.donor === selectedMember.name);
                const totalMemberTithes = memberTithes.reduce((s: number, t: any) => s + parseFloat(t.amount || 0), 0);
                return (
                  <div>
                    ...
                    <div className="bg-emerald-50 rounded-xl p-3 mb-3 flex justify-between items-center">
                      <span className="text-emerald-700 font-semibold text-sm">Total Tithes Given</span>
                      <span className="text-emerald-800 font-bold text-lg">GH₵{totalMemberTithes.toFixed(2)}</span>
                    </div>
                    {memberTithes.length === 0 ? (
                      <p className="text-gray-400 text-sm text-center py-3 italic">No tithe records yet</p>
                    ) : (
                      <div className="space-y-2">
                        {[...memberTithes].sort((a,b) => b.date.localeCompare(a.date)).map((t: any) => (
                          <div key={t.id} className="flex justify-between items-center bg-gray-50 rounded-lg px-3 py-2 text-sm">
                            <div>
                            ...
                            <span className="font-bold text-emerald-700">GH₵{parseFloat(t.amount).toFixed(2)}</span>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {modalType === 'weeklySheet' && (
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Sunday Date</label>
                    <select value={weeklySheetForm.date} onChange={e => setWeeklySheetForm({...weeklySheetForm, date: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                      <option value="">Select Sunday</option>
                      {SUNDAYS_2026.map(s => <option key={s} value={s}>{new Date(s + 'T00:00:00').toLocaleDateString('en-GH', { weekday:'long', day:'2-digit', month:'long', year:'numeric' })}</option>)}
                    </select>
                  </div>
                  {[
                    { key: 'tithes', label: 'Tithes (GH₵)' },
                    { key: 'dues', label: 'Dues (GH₵)' },
                    { key: 'funeralContributions', label: 'Funeral Contributions (GH₵)' },
                    { key: 'sundaySchool', label: 'Sunday School Offering (GH₵)' },
                    { key: 'bookSales', label: 'Book Sales (GH₵)' },
                  ].map(f => (
                    <div key={f.key}>
                      <label className="block text-sm font-medium text-gray-700 mb-1">{f.label}</label>
                      <input type="number" step="0.01" min="0" placeholder="0.00"
                        value={(weeklySheetForm as any)[f.key]}
                        onChange={e => setWeeklySheetForm({...weeklySheetForm, [f.key]: e.target.value})}
              ...
              {modalType === 'expenditure' && (
                <>
                  <div>
                    ...
                    <input type="date" value={expenditureForm.date} onChange={e => setExpenditureForm({...expenditureForm, date: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <input type="text" placeholder="What was the expenditure for?" value={expenditureForm.description} onChange={e => setExpenditureForm({...expenditureForm, description: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Amount (GH₵)</label>
                    <input type="number" step="0.01" min="0" placeholder="0.00" value={expenditureForm.amount} onChange={e => setExpenditureForm({...expenditureForm, amount: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
                    <select value={expenditureForm.category} onChange={e => setExpenditureForm({...expenditureForm, category: e.target.value})} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
